...
{
    public class QuandoForExecutadogetByCep : CepTestes
    {
        private ICepService _cepService;
        private Mock<ICepService> _serviceMock;

        [Fact(DisplayName = "É possível executar Método Get By Cep")]
        public async Task E_Possivel_Executar_Metodo_Get_By_Cep()
        {
            ...
            _serviceMock = new Mock<ICepService>();
            _serviceMock.Setup(c => c.Get(CodCep)).ReturnsAsync(cepDto);
            _cepService = _serviceMock.Object;

            result = await _cepService.Get(CodCep);
            Assert.NotNull(result);
            Assert.True(result.Id == IdCep);
            Assert.Equal(CodCep, result.Cep);
            Assert.Equal(LogradouroCep, result.Logradouro);
